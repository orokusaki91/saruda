...
class PrehistoryArticleController extends Controller
{
    public function index(Workshop $article)
    {
        $records = $article->articles;
        return view('admin.pages.workshop.prehistory.article.show', compact('records', 'article'));
    }

    public function create(Workshop $article)
    {
        return view('admin.pages.workshop.prehistory.article.create', compact('article'));
    }
    ...
    public function store(ArticleStoreRequest $request, Workshop $article)
    {
        ...
        return redirect()->route('admin.workshop.prehistory.article', ['article' => $article->id])->with(['success' => 'You successfuly created new record']);
    }

    public function edit(WorkshopArticle $record)
    {
        return view('admin.pages.workshop.prehistory.article.edit', compact('record'));
    }
    ...
    public function update(ArticleUpdateRequest $request, WorkshopArticle $record)
    {
        ...
        return redirect()->route('admin.workshop.prehistory.article', ['article' => $record->workshop_id])->with(['success' => 'You successfuly updated the record']);
    }
    ...
    public function destroy(WorkshopArticle $record)
    {
        ...
        return redirect()->route('admin.workshop.prehistory.article', ['article' => $redirect_param])->with(['success' => 'You successfuly deleted the record']);
    }
}
